...
function checkAuth(req, res, next) {
    ...
    if (validTokens.hasOwnProperty(token)) {
        ...
        if (now <= expireDate) {
            ...
            res.status(401).json({ success: false, error: `⛔ 授權已於 ${expireDateStr} 到期，請聯絡管理員。` });
        }
    } else {
        res.status(401).json({ success: false, error: '⛔ 授權碼無效，請確認輸入是否正確。' });
    }
}
...
app.post('/api/calculate', checkAuth, (req, res) => {
    ...
    if (!currentDB[key]) {
        return res.json({ success: false, error: `查無此規格: ${t} x ${f} x ${m}` });
    }

    const typeData = currentDB[key][type] || currentDB[key]['TB'];
    if (type === 'Pointed' && !typeData) {
        return res.json({ success: false, error: "此規格無「尖尾」價格" });
    }
    ...
    if (options.complex50) { pct += 0.50; logs.push("特殊牙型 (+50%)"); }
    if (options.pitch04) { pct += 0.15; logs.push("牙距0.4以下 (+15%)"); }
    if (options.twoSpecs) { pct += 0.30; logs.push("雙規格 (+30%)"); }
    if (options.doubleLeft) { pct += 0.10; logs.push("雙牙/左牙 (+10%)"); }
    if (options.angle3048) { pct += 0.10; logs.push("牙山30-48度 (+10%)"); }
    if (options.faces4) { pct += 0.50; logs.push("四面用 (+50%)"); }
    if (options.surface) { pct += 0.15; logs.push("表面處理 (+15%)"); }
    if (options.straight42) { pct += 0.30; logs.push("直齒42牙+ (+30%)"); }
    ...
    // 美金換算 (假設匯率為 32，您可自行修改)
    const usdRate = 32.0;
    if (currency === 'USD') {
        let oldTWD = total;
        total = total / usdRate;
        logs.push(`轉換美金 (匯率 1:${usdRate}): TWD ${Math.round(oldTWD).toLocaleString()} -> USD ${total.toFixed(2)}`);
    }
...
app.listen(PORT, () => console.log(`伺服器運作中: Port ${PORT}`));